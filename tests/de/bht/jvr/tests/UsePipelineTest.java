 ...
 */
public class UsePipelineTest extends TestBase {
    ...
    public UsePipelineTest() throws IOException {
        ...
        Pipeline renderP = new Pipeline(null);

        // render to fbo
        renderP.drawGeometry("AMBIENT", null);
        renderP.doLightLoop(true, true).drawGeometry("LIGHTING", null, false);
        // p.doForwardLightLoop("LIGHTING", null, false);

        // switch to p-buffer
        renderP.switchFrameBufferObject(null);
        // clear p-buffer
        renderP.clearBuffers(true, true, new Color(0, 0, 0));

        // set uniforms & buffers
        PipelineCommandPtr ptr = renderP.setUniform("intensity", new UniformFloat(intensity));
        renderP.bindColorBuffer("jvr_Texture1", "fbo1", 0);
        renderP.bindDepthBuffer("jvr_Texture0", "fbo1");

        // render quad
        renderP.drawQuad(sm, "PASS0");

        Pipeline setupP = new Pipeline(root);

        // create fbo
        setupP.createFrameBufferObject("fbo1", true, 1, 1.0f, 4);

        // switch to fbo
        setupP.switchFrameBufferObject("fbo1");
        // clear fbo buffers
        setupP.clearBuffers(true, new boolean[] { true }, Color.gray);

        setupP.switchCamera(cam1);

        setupP.usePipeline(renderP);

        // /////////////////////////////////////////////

        RenderWindow w = new AwtRenderWindow(setupP, 800, 600);
