 ...
 */
public class DynamicVBOTest extends TestBase {
    ...
    public DynamicVBOTest() throws Exception {
        GroupNode root = new GroupNode();

        SceneNode scene = ColladaLoader.load(new File("data/meshes/teapot.dae"));
        scene.setTransform(Transform.translate(0, -10, 0).mul(Transform.scale(20)));
        ShapeNode shape = Finder.find(scene, ShapeNode.class, null);
        TriangleMesh mesh = (TriangleMesh) shape.getGeometry();
        ...
        List<Vector4> vertices = mesh.getVertices();
        try {
            // boolean init = false;
            while (viewer.isRunning() && run) {
                ...
                // if(!init)
                {
                    for (int i = 0; i < vertices.size() / 2; i++) {
                        Vector4 v = vertices.get(i);
                        vertices.set(i, new Vector4(v.x(), v.y(), (float) Math.random(), 1));
                    }
