 ...
 */
public class CaveHeadTrackingTest2 extends TestBase {
    public static void main(String[] args) {
        Log.addLogListener(new LogPrinter(-1, 0, -1));
        try {
            new CaveHeadTrackingTest2();
        } catch (IOException e) {
    ...
    public CaveHeadTrackingTest2() throws IOException {
    ...
    public Pipeline makePipeline(SceneNode root, LightNode sLight, CameraNode cam) throws IOException {
        ...
        // create fbo for dof
        p.createFrameBufferObject("DOFMap", true, 1, 1.0f, 0);
    ...
    protected void move(long renderDuration) {
        ...
        double speed = renderDuration / 10.;
        synchronized (pressedKeys) {
            for (Character key : pressedKeys)
                switch (key) {
                case 'E':
                    intensity -= 0.1f * speed;
                    if (intensity < 0)
                        intensity = 0;
                    break;
                case 'R':
