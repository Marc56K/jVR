...
public class UniformColor implements UniformValue {
    /** The color values. */
    private List<Color> values = new ArrayList<Color>();
    private float[] finalData;

    /**
     * Instantiates a new uniform color.
     * 
     * @param values
     *            the values
     */
    public UniformColor(Collection<Color> values) {
     ...
     *            the values
     */
    public UniformColor(Color... values) {
        for (Color val : values)
            this.values.add(val);
        createDataArray();
    }
    ...
    public boolean equals(Object val) {
        if (val instanceof UniformColor) {
            for (int i = 0; i < values.size(); i++)
                if (!values.get(i).equals(((UniformColor) val).values.get(i)))
                    return false;
            return true;
        }
        return false;
