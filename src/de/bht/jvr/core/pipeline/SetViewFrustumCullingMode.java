...
public class SetViewFrustumCullingMode implements PipelineCommand {
    private int mode;

    public SetViewFrustumCullingMode(int mode) {
        this.mode = mode;
    }

    @Override
    public void execute(Context ctx, PipelineState pipelineState) throws Exception {
        pipelineState.setViewFrustumCullingMode(mode);
    }
    ...
    public void update(PipelineState pipelineState) {
        if (pipelineState.getViewFrustumCullingMode() != mode)
            pipelineState.setActiveGeoList(null);
