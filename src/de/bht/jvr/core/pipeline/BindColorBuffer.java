...
public class BindColorBuffer implements PipelineCommand {
    private String uniformSampler;
    private String fboName;
    private int bufIndex;

    public BindColorBuffer(String uniformSampler, String fboName, int bufIndex) {
    ...
    public void execute(Context ctx, PipelineState pipelineState) throws Exception {
        ...
        if (fbo != null) {
            Texture2D tex = fbo.getTexture2D(ctx, GL.GL_COLOR_ATTACHMENT0 + bufIndex);
            if (tex != null)
                pipelineState.setTexture2D(uniformSampler, tex);
            else
                Log.error(this.getClass(), "No color buffer with index " + bufIndex + " in FBO " + fboName);
        } else
