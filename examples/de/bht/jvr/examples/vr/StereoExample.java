...
public class StereoExample implements MouseListener {
    /**
     * @param args
     */
    ...
    private CameraNode camLeft;
    private CameraNode camRight;
    private GroupNode root;
    private Vector2 mousePos = new Vector2(0, 0);

    public StereoExample() throws Exception {
        // load geometry
        ...
        ShaderProgram lightingProg = new ShaderProgram(new File("data/shader/bumpmapping.vs"), new File("data/shader/parallaxmapping.fs"));

        // create the shader material
        ShaderMaterial mat = new ShaderMaterial();
        mat.setShaderProgram("AMBIENT", ambientProg); // set the ambient shader program
        mat.setUniform("AMBIENT", "jvr_Material_Ambient", new UniformColor(new Color(0, 0, 0, 1))); // set ambient color of the material to black
        mat.setShaderProgram("LIGHTING", lightingProg); // set the lighting shader program
        // load textures
        mat.setTexture("LIGHTING", "MyNormalMap", new Texture2D(new File("data/textures/layingrock-normals.jpg")));
        mat.setTexture("LIGHTING", "MyTextureMap", new Texture2D(new File("data/textures/layingrock.jpg")));
        mat.setTexture("LIGHTING", "MyHeightMap", new Texture2D(new File("data/textures/layingrock-height.jpg")));
        mat.setUniform("LIGHTING", "MyHeightScale", new UniformFloat(0.09f));
        // set the material
        ...
        mat.setUniform("LIGHTING", "jvr_Material_Shininess", new UniformFloat(30));

        // create the shape node
        ShapeNode shape = new ShapeNode("MyShape", geo, mat);

        // create a camera
        camLeft = new CameraNode("MyCameraLeft", 4f / 3f, 60);
        camLeft.setTransform(Transform.translate(-0.5f, 0.5f, 1.75f)); // transform the camera

        camRight = new CameraNode("MyCameraRight", 4f / 3f, 60);
        camRight.setTransform(Transform.translate(0.5f, 0.5f, 1.75f)); // transform the camera
        ...
        root.addChildNodes(shape, camLeft, camRight, pLight);
        ...
        p.switchCamera(camLeft);
        p.switchFrameBufferObject(null);
        p.setDrawBuffer(GL2GL3.GL_BACK_LEFT);
        ...
        p.switchCamera(camRight);
        p.setDrawBuffer(GL2GL3.GL_BACK_RIGHT);
        p.clearBuffers(true, true, new Color(0, 0, 0));
        lp = p.doLightLoop(true, true);
        ...
        while (v.isRunning()) {
            ...
            angle += (System.currentTimeMillis() - start) * 0.0002;
        }
    }
    ...
    public void updateLightPos() {
        ...
        PickRay ray = Picker.getPickRay(root, camLeft, mousePos.x(), mousePos.y());
